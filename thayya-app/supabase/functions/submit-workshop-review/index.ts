import {
  corsHeaders,
  requireAuthenticatedUser,
} from "../_shared/require-authenticated-user.ts";

const LOG = "submit-workshop-review";

const uuidRe =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

function isUuid(s: string): boolean {
  return uuidRe.test(s.trim());
}

type Body = {
  workshop_id?: string;
  rating?: number;
};

type ReviewRow = {
  id: string;
  workshop_id: string;
  user_id: string;
  instructor_id: string;
  rating: number;
  created_at: string;
  updated_at: string;
};

const REVIEW_COLS =
  "id, workshop_id, user_id, instructor_id, rating, created_at, updated_at";

function isWorkshopPast(dateValue: string | null): boolean {
  if (dateValue == null || dateValue === "") return false;
  const when = new Date(dateValue);
  return Number.isFinite(when.getTime()) && when.getTime() < Date.now();
}

Deno.serve(async (req: Request) => {
  if (req.method === "OPTIONS") {
    return new Response("ok", { headers: corsHeaders });
  }

  if (req.method !== "POST") {
    return Response.json(
      { error: "Method not allowed" },
      { status: 405, headers: corsHeaders },
    );
  }

  const gate = await requireAuthenticatedUser(req, LOG);
  if (gate instanceof Response) return gate;
  const { admin, user } = gate;

  let body: Body;
  try {
    body = (await req.json()) as Body;
  } catch {
    return Response.json(
      { error: "Invalid JSON body" },
      { status: 400, headers: corsHeaders },
    );
  }

  const workshopId =
    typeof body.workshop_id === "string" && body.workshop_id.trim()
      ? body.workshop_id.trim()
      : "";
  if (!workshopId || !isUuid(workshopId)) {
    return Response.json(
      { error: "workshop_id (uuid) is required" },
      { status: 400, headers: corsHeaders },
    );
  }

  const ratingRaw = body.rating;
  const rating =
    typeof ratingRaw === "number"
      ? ratingRaw
      : typeof ratingRaw === "string"
        ? Number(ratingRaw)
        : NaN;
  if (!Number.isInteger(rating) || rating < 1 || rating > 5) {
    return Response.json(
      { error: "rating must be an integer from 1 to 5" },
      { status: 400, headers: corsHeaders },
    );
  }

  const { data: workshop, error: workshopErr } = await admin
    .from("workshops")
    .select("id, date, instructor_id")
    .eq("id", workshopId)
    .maybeSingle();

  if (workshopErr) {
    console.error(`[${LOG}] load workshop`, workshopErr.message);
    return Response.json(
      { error: workshopErr.message },
      { status: 500, headers: corsHeaders },
    );
  }
  if (!workshop) {
    return Response.json(
      { error: "Workshop not found" },
      { status: 404, headers: corsHeaders },
    );
  }

  if (!isWorkshopPast(workshop.date != null ? String(workshop.date) : null)) {
    return Response.json(
      { error: "You can only rate a workshop after it has taken place" },
      { status: 400, headers: corsHeaders },
    );
  }

  const instructorId =
    typeof workshop.instructor_id === "string" ? workshop.instructor_id : "";
  if (!instructorId) {
    return Response.json(
      { error: "This workshop has no instructor to rate" },
      { status: 400, headers: corsHeaders },
    );
  }

  if (instructorId === user.id) {
    return Response.json(
      { error: "You cannot rate your own workshop" },
      { status: 400, headers: corsHeaders },
    );
  }

  const { data: registration, error: regErr } = await admin
    .from("workshop_registrations")
    .select("id, status")
    .eq("workshop_id", workshopId)
    .eq("user_id", user.id)
    .maybeSingle();

  if (regErr) {
    console.error(`[${LOG}] registration`, regErr.message);
    return Response.json(
      { error: regErr.message },
      { status: 500, headers: corsHeaders },
    );
  }
  if (!registration || registration.status !== "active") {
    return Response.json(
      { error: "Only attendees who registered for this workshop can leave a rating" },
      { status: 403, headers: corsHeaders },
    );
  }

  const nowIso = new Date().toISOString();
  const { data: existing } = await admin
    .from("workshop_reviews")
    .select("id")
    .eq("workshop_id", workshopId)
    .eq("user_id", user.id)
    .maybeSingle();

  let review: ReviewRow | null = null;
  let error: { message: string; code?: string } | null = null;

  if (existing?.id) {
    const updated = await admin
      .from("workshop_reviews")
      .update({ rating, updated_at: nowIso })
      .eq("id", existing.id)
      .select(REVIEW_COLS)
      .single<ReviewRow>();
    review = updated.data;
    error = updated.error;
  } else {
    const inserted = await admin
      .from("workshop_reviews")
      .insert({
        workshop_id: workshopId,
        user_id: user.id,
        instructor_id: instructorId,
        rating,
      })
      .select(REVIEW_COLS)
      .single<ReviewRow>();
    review = inserted.data;
    error = inserted.error;
  }

  if (error || !review) {
    console.error(`[${LOG}] save`, error?.message ?? "unknown");
    return Response.json(
      { error: error?.message ?? "Could not save rating" },
      { status: 400, headers: corsHeaders },
    );
  }

  return Response.json({ review }, { headers: corsHeaders });
});
